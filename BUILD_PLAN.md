...
| 16 | **E** | テキスト整合パス（公式テキスト書き起こしとの差分補正） → §7 | テキスト入手後 | 済 | 2026-08-30 | `f8b1c69` | Sec27を暫定7問→公式**5つの問い**へ全面差し替え（責任分界点3者・業務シナリオ追加）。Sec18を公式名へ改題し欠落していた核（二重構造・PoC定性/本運用定量・逆算3観点・使いたくない4分類・成功定義シート）を加筆。QB7問を差し替え（ID/sec/ch/type維持のため模試125問の割付は不変）。用語82語。`tools/qcheck.js` を追加し強い手がかり0件・型3順位語欠落0件を確認 |
