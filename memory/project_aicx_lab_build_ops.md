...
aicx-lab（AICX資格の学習サイト）は **1セッション1Build** で進める。※ 2026-08-28 は HK の連続指示により B3〜B7 の5本を1セッションで実施したが、**HK は「一気にやらず1つずつ」を明示選択した**（残Buildを一括処理する提案を検討したうえでの判断）。以後は原則どおり1つずつ。進行状態の正は `BUILD_PLAN.md` の進行表、仕様の正は `DESIGN.md`。HK の「次の Build をやって」は「進行表の未着手の最上行を実装せよ」の意味。
...
関連: [[feedback_token_discipline]] / [[project_aicx_lab_question_quality]] / [[feedback_announce_before_work]]
