...
- Build B7（第7回 Sec19-22 自動化レベルとワークフロー設計）まで完了し、本番 https://kannohi1.github.io/aicx-lab/ で稼働中。B3〜B7 の5本を 2026-08-28 の1セッションで前倒し実施し、進行表の目安日より約1週間先行
- サイトの実体: QB 142問（pool both 70 / mock 72）・用語 64語・レッスン公開 7/14回。localStorage スキーマ v2（SRS・daily・streak）で v1 からの自動移行あり
- 残り Build は 9個（B8〜B11・C1・C2・D1・D2 の8個 + テキスト待ちの E）。HK は「一気にやらず1つずつ」の方針を選択（2026-08-28）
- **HK の実務タスク: 受験申込（締切 2026-09-20・残り約3週間）と公式テキスト購入。第6回の教材末尾に CTA を設置済み**
- 次は Build B8（第8回 Sec23-24 組織文化と推進体制。公式サイトの5課題を組織文化の観点から再整理）。**着手前に「次は Build B8、着手します」と HK へ一言伝えてから始めること**
- 最新リモート同期: 2026-08-29 00:16 JST — origin/main = HEAD (ahead 0 / behind 0)

## 次にやること

1. Build B8: 第8回 Sec23-24（組織文化・推進体制・公式5課題の再整理）→ DESIGN.md §4「第8回」 [Claude]
2. 以降 B9〜B11 を進行表の順に 1セッション1Build で消化 [Claude]
3. Build C1（模試エンジン＋弱点マップ）— 弱点マップは Build A で実装せず C1 へ据え置いた（設計どおり）。**コードなので作問Buildとは別の検証が要る** [Claude]
4. 既存 Build（第1〜4回）の長さ手がかりを node スクリプトで一括是正 — 第2回13件・第1回4件・第3回6件・第4回4件が未修正のまま残存。B8 のついでか単発で片付ける [Claude]
5. B8〜D2 完了後、横断の敵対的レビュー（Opus → Fable×max の見落とし探し）。**Fable への切替は HK が `/model` で行う必要があるためリマインドする** [Claude]
